...
impl Qwen3Tokenizer {
    pub fn new(model_dir: &Path) -> Result<Self, Qwen3Error> {
        let tokenizer_path = model_dir.join("tokenizer.json");
        let file = fs::File::open(&tokenizer_path)?;
        let reader = std::io::BufReader::new(file);
        let json: serde_json::Value =
            serde_json::from_reader(reader).map_err(|e| Qwen3Error::Tokenizer(e.to_string()))?;
        ...
        if id_to_bytes.is_empty() {
            return Err(Qwen3Error::Tokenizer(
